...
public class Sprite3DActor extends SpriteActor {
	...
	// This implies that the shadow must be calculated in the draw method and not in the update
	...
	public void draw(SpriteBatch batch, float x, float y, float originX,
		...
		if (USE_FBO) {
			super.draw(batch, x, y, originX, originY, scale);
		} else {
			batch.end();

			Gdx.gl20.glViewport((int) (x + VIEWPORT.x), (int) (y + VIEWPORT.y), (int) (width * scale),
					(int) (height * scale));
	...
	public void draw(SpriteBatch batch) {
		if (isVisible()) {
			if (USE_FBO) {
				...
				Vector3 p0 = c.scene2screen(pos.x - getWidth() / 2, pos.y, VIEWPORT);
				Vector3 pf = c.scene2screen(pos.x - getWidth() / 2 + getWidth() , pos.y + getHeight(), VIEWPORT);						

				Gdx.gl20.glViewport((int) (p0.x + VIEWPORT.x), (int) (p0.y + VIEWPORT.y), (int) (pf.x - p0.x),
						(int) (pf.y - p0.y));
