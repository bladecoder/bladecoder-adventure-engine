...
public class Project extends PropertyChange {
    ...
    private final HashSet<String> hiddenActors = new HashSet<>();
    ...
    public void toggleEditorVisibility(BaseActor a) {
        ...
        if (hiddenActors.contains(name)) {
            hiddenActors.remove(name);
        } else {
            hiddenActors.add(name);
        }
    }
    ...
    public boolean isEditorVisible(BaseActor a) {
        ...
        return !hiddenActors.contains(name);
    }
}
